...
<body class="bg-slate-950 text-slate-100">
  <div class="max-w-6xl mx-auto p-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <section class="rounded-2xl border border-slate-800 bg-slate-900/60 shadow">
        ...
        <div id="obs-list" class="p-5 text-sm text-slate-300 font-mono grid gap-1">
          Loading…
        </div>
      </section>
    </div>
    ...
    try{
      ...
      if (Array.isArray(s)){
        ...
        for(const m of Object.keys(s).sort()){
          const r = s[m]||{};
          rows.push([m, r.count ?? '', r.sum ?? '', r.avg ?? '', r.last_ts ?? '']);
        }
      }
      $('stats-tbody').innerHTML = rows.map(r=>(
        '<tr class="hover:bg-slate-800/40 transition">'
          ...
          + '<td class="py-2 pl-2 text-right font-mono">'+r[4]+'</td>'
        + '</tr>'
      )).join('') || '<tr><td colspan="5" class="py-4 text-slate-400">No data</td></tr>';
    }catch(e){
      $('stats-err').textContent = 'Stats error: ' + e.message;
    }
  }
  ...
  async function load(){
    try{
      const r = await fetch('/v1/bandit/stats'); renderStats(await r.json());
    }catch(e){ $('stats-err').textContent = 'Stats fetch failed'; }
    try{
      const r2 = await fetch('/v1/bandit/observations?limit=100');
      if(r2.ok){ renderObs(await r2.json()); } else { throw new Error('obs not ok'); }
    }catch(e){ $('obs-err').textContent = 'Observations endpoint not available'; $('obs-list').innerHTML = '<div class="text-slate-400">—</div>'; }
  }
  load();
  </script>
</body>
